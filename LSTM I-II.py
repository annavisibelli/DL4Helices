...
for run in range(20): #multiple run
    ...
    path_nonhelices_1h = "./Data/Not_HelicesOneHot.csv" #file of input non-helix sequences in one-hot encoding
    path_nonhelices_w2v = "./Data/Not_Helicesw2v.csv" #file of input non-helix sequences in word2vec encoding
    ...
    hu_lstm_w2v = 43 #hidden units in the LSTM layer
    hu_dense_w2v = 10 #hidden units in the dense layer 
    hu_lstm_1h = 38 #hidden units in the LSTM layer
    ...
    #split non-helix data into training and test set
    ...
    for k in range(10):
        ...
        layer_lstm = layers.LSTM(return_sequences=True, activation="selu", units=hu_lstm, recurrent_activation="sigmoid", kernel_regularizer = regularizers.l1(0.01), bias_regularizer = regularizers.l1(0.01))(layer_input)
        layer_attention_input = layers.Permute((2, 1))(layer_lstm)
        ...
        layer_multiplication = layers.multiply([layer_lstm, layer_attention_scores])
        layer_flattened = layers.Flatten()(layer_multiplication)
        layer_hidden = layers.Dense(units=hu_dense, activation='selu', kernel_regularizer = regularizers.l1(0.01), bias_regularizer = regularizers.l1(0.01))(layer_flattened)
